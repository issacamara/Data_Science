...
def answer_two():
    ...
    recall_score = (recall_score(y_dummy_predictions, y_test))
...
def answer_four():
    ...
    y_score_list = list(zip(y_test, y_pred, y_scores_svm))
    
    y_test_filtered = [a for a,b,c in y_score_list if c <= -220]
    y_pred_filtered = [b for a,b,c in y_score_list if c <= -220]
    
    y_pred = [0 if score < -220 else 1 for score in y_scores_svm]
