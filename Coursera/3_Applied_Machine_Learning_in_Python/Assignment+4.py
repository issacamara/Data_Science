...
def blight_model():
    ...
    predictors = ['fine_amount', 'admin_fee', 'state_fee', 'late_fee', 
                  'discount_amount', 'clean_up_cost', 'judgment_amount']
    
    
    X_blight = train_df[predictors]
    ...
    y_score = model.fit(X_train_scaled, y_train).predict_proba(X_test_scaled)
    ...
    predictions = pd.Series(index=test_df['ticket_id'], data=probs[:,1], name='compliance')
